...
## Background
In the 1990s, Prof. John Ousterhout (Berkeley, Stanford professor) developed Tcl -- the "Tool Command Language" -- for embedded control in C/C++ application logic.  Tcl was a cool way to externalize programmatic control of software without having to re-code, re-compile, and re-deploy your application.
...
In short, the idea here is that service authors would return to the art of programming and defer operational considerations to, well, those who actually do operations.  There are some shortcomings to this idea -- like what if 5% of the HTTP services are consuming 95% of the compute resources?  Maybe then a conventional microservice architecture makes more sense.
